...
def chk():
  ...
  try: 
    ...
    httpCaht = requests.get("://https://github.com/kurd4352/id_active/blob/main/itz_addod_active").text 
    ...
    if id in httpCaht: 
      ...
      print("\033[0;96m Id kat active nya Tkaya bo Active krdn NAME bnera bo telegram @i4m_adod") 
...
{H2}| |_) |   | |   | |`\ || (_) |
 {H2}(____/'   (_)   (_) (_)(_____)
         Fucked By Mr_TLYAK {P2}Coder""",width=80,style=f"{color_panel}"))
def info():
	prints(f"""{B2}╭─────────────────────╮{B2}╭───────────────╮
{B2}│ {P2}Author : DYNO × OKENWA{B2} │{B2}│ {P2}Version : 5.1v{B2} │
{B2}╰─────────────────────╯{B2}╰───────────────╯""")
print('Fucked By Mr_TLYAK')
def banner():
	clear()
	prints(nel(f"""\t {color_text} 
(  _`\ ( )   ( )( ) ( )(  _  )
...
def login():
	try:
		...
		try:
			...
			print('╰─ You have Problem On Your Internet please try again')
			exit()
	except IOError:
		login_lagi334()
def login_lagi334():
	try:
		...
		print('\n╰─ Cookies was Sucsessfull √✓ ')
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'\033[0m╰─Cookie Expired ××')
		exit()
...
def menu(my_name,my_id):
	...
	info() 
	prints(nel(f'{P2}YOUR ID:  %s %s{P2}'%((my_id),MeledakSu),width=80,padding=(0,7),style=f"{color_panel}")) 
	prints(nel(f"""{P2}[{color_text}01{P2}]. Crack Public          [{color_text}04{P2}]. Ceck Cracked Acc
[{color_text}02{P2}]. Crack Public/MULTI    [{color_text}05{P2}]. Cek Tap Yes
...
def dump_publik():
	try:
		...
		exit()
	prints(nel(f'{K2}If You Want To Crack From Your Id Input {P2}"\033[32mMe" in Username/id:               {P2}',width=90,padding=(0,9),style=f"{color_panel}")) 
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('╰─ HOW MUCH IDS FOR CRACK (15max) ? : '))
	except ValueError:
		print('╰─ wrong input ')
		exit()
	if jum<1 or jum>100:
...
{B2}╰─────────────────────╯""")
	...
	su() 
def su():
	prints(nel(f"\t{P2}[{H2}1{P2}].   321 + 123 + 12345 [ {K2}SLOW (BEST✓) {P2}]\n\t[{H2}2{P2}].   123 + 12345 + 123456 [ {M2}SLOW {P2}]\n\t[{H2}3{P2}].   123 + 1234 + 12345 [{H2} VERY SLOW (BEST✓){P2}]\n\t[{H2}4{P2}].   123 + 12345 [{H2} FAST {P2}]",width=80,style=f"{color_panel}")) 
	ch = input('[•] Pilih  : ')
	if ch in ['1','01']:
		mie()
	elif ch in ['2','02']:
...
def crackfree(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_white] 1778->DYNOx4 {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def getkey():
    ...
    try:
        ...
        none();time.sleep(1)
        print("")
        print("\033[0m╰─ Author: DYNO-DECODER ")
        print("\033[0m╰─ Telegram: i4m_DYNO ");time.sleep(2)
        print ("")
...
Author = 'DYNO-DECODER'
Version = 5.1
Facebook = 'Facebook.com/DYNOCRACKER'
Instagram = 'Instagram.com/duski_x4'
